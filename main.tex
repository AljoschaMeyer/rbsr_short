...
\subsection{Range-Based Set Mirroring}
...
The procedure \textproc{aggregate\_left} demonstrates how to compute $\lift{\f}{\mathcal{M}}\bigl(\set{z \in \V(\tl{init}) \mid z \succeq x}\bigr)$: starting from the initial vertex, we search for $x$, accumulating the labels of all right children, as well as the monoid values that correspond to those vertices on the search path that are greater than or equal to $x$. Similarly, the procedure \textproc{aggregate\_right} computes $\lift{\f}{\mathcal{M}}\bigl(\set{z \in \V(\tr{init}) \mid z \prec y}\bigr)$. \Cref{fig:aggregate_once} depicts an example run.
...
We distinguish between \defined{active} adversaries who can freely select the sets to be reconciled, and \defined{passive} adversaries who needs to find and cause collisions in existing sets. If fingerprints are bit strings of length $k$, every set of size at least $k + 1$ contains two subsets with the same fingerprint. We primarily focus on active adversaries, as they are more powerful.
...
\begin{definition}
A \defined{secure hash function} is a hash function $\fun{\h}{U}{D}$ that satisfies three additional properties~\cite{menezes2018handbook}:
...
All of these hash functions are \defined{multiset homomorphic}~\cite{clarke2003incremental}:
...
% There is a large number of engineering issues and possible generalizations, for example,
